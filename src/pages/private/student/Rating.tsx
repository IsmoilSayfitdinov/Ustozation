...
import { LEADERBOARD } from '@/data/student';
...
const StudentRating = () => {
  ...
  const topThree = useMemo(() => {
    // Sardor (rank 1), Nodira (rank 2), Jasur (rank 3)
        ...
        <div className="space-y-8">
          ...
          <div className="space-y-4">
            {paginatedRating.length > 0 ? (
              paginatedRating.map((student, index) => (
                <LeaderboardRow 
                  key={index}
                  rank={student.rank}
                  name={student.name}
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-center gap-2 pt-4">
              <button 
                ...
                className="w-10 h-10 rounded-xl bg-white border border-[#F2F4F7] flex items-center justify-center text-[#141F38] hover:border-surface-tint/20 hover:text-surface-tint disabled:opacity-30 disabled:cursor-not-allowed transition-all cursor-pointer"
              >
                  ...
                  <button
                    ...
                    className={`w-10 h-10 rounded-xl font-bold text-sm transition-all cursor-pointer ${
                      currentPage === page 
                        ...
                        : 'bg-white border border-[#F2F4F7] text-[#141F38] hover:border-surface-tint/20 hover:text-surface-tint'
                    }`}
                  >
                    {page}
                  </button>
              ...
              <button 
                ...
                className="w-10 h-10 rounded-xl bg-white border border-[#F2F4F7] flex items-center justify-center text-[#141F38] hover:border-[#F9731633] hover:text-[#F97316] disabled:opacity-30 disabled:cursor-not-allowed transition-all cursor-pointer"
              >
