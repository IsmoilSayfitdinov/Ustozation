...
import { createLessonSchema, type CreateLessonSchema } from '@/schemas/private/admin/lesson';
...
const CreateLessonDialog: React.FC<CreateLessonDialogProps> = ({ isOpen, onClose, onSubmit, isPending }) => {
  const { register, handleSubmit, reset, formState: { errors } } = useForm<CreateLessonSchema>({
  ...
  const onFormSubmit = (data: CreateLessonSchema) => {
        ...
        <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className="relative w-full max-w-md bg-[#F9FAFB] rounded-[24px] shadow-2xl overflow-hidden flex flex-col">
          <div className="px-6 pt-6 pb-4 flex items-center justify-between">
            <div className="flex items-center gap-2 text-[#141F38]">
              ...
              <h2 className="text-xl font-headline font-black tracking-tighter">Yangi dars qo'shish</h2>
            </div>
          ...
          <form onSubmit={handleSubmit(onFormSubmit)} className="p-6 pt-0 space-y-5">
            <div className="space-y-1.5">
              ...
              <input {...register('title')} type="text" placeholder="Masalan: Present Simple" className={`w-full px-4 py-3 bg-white border rounded-xl outline-none focus:ring-2 focus:ring-[#F97316]/20 transition-all font-medium text-[#1C2434] text-sm ${errors.title ? 'border-red-400' : 'border-[#E4E7EC]'}`} />
            ...
            <div className="space-y-1.5">
              ...
              <textarea {...register('description')} placeholder="Qisqacha tavsif..." className="w-full px-4 py-3 bg-white border border-[#E4E7EC] rounded-xl outline-none focus:ring-2 focus:ring-[#F97316]/20 transition-all font-medium text-[#1C2434] text-sm min-h-[80px] resize-none" />
            </div>
